...
import at.fhooe.mc.datadora.RoomDatabase.DoubleLinkedListRoom;
import at.fhooe.mc.datadora.RoomDatabase.SingleLinkedListRoom;

public class DoubleLinkedListView extends View implements ValueAnimator.AnimatorUpdateListener, Animator.AnimatorListener {


    private static final String TAG = "DoubleLinkedListView : ";
    ...
    private DoubleLinkedListActivity mLinkedListActivity;
    ...
    public Vector<Integer> getDoubleLinkedListNumbers() {
    ...
    public DoubleLinkedListView(Context context) {
        super(context);
        init();
    }

    public DoubleLinkedListView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public DoubleLinkedListView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void setActivity(DoubleLinkedListActivity _activity) {
    ...
    private void loadDoubleLinkedListFromDatabase(){

        mDatadoraViewModel.getmAllDoubleLinkedListValues().observe(mLinkedListActivity, listRooms -> {

            if(mLinkedListNumbers.isEmpty() && mCurrentOperation != Operation.RANDOM) {
                for (DoubleLinkedListRoom dllr : listRooms) {
                    mLinkedList.add(new RectF());
                    mLinkedListNumbers.add(dllr.getVal());
    ...
    protected void prepend(int _value){
        ...
        mDatadoraViewModel.insertFirst(new DoubleLinkedListRoom(_value));
        reScale();
    ...
    protected void append(int _value){
        ...
        mDatadoraViewModel.insertLast(new DoubleLinkedListRoom(_value));
        reScale();
    ...
    protected void clear(){
        ...
        mAnimatorDeleteLast.start();
        reScaleUndo();
        mDatadoraViewModel.deleteAllDoubleLinkedListDatabaseEntries();
    }
    ...
    protected void random(Vector<Integer> _list) {
        ...
        for (int i = 0; i < _list.size(); i++){
            mDatadoraViewModel.insertLast(new DoubleLinkedListRoom(_list.elementAt(i)));
        }
    ...
    protected void onSizeChanged(int _w, int _h, int _oldW, int _oldH) {
        ...
        mLinkedList.clear();
        mLinkedListNumbers.clear();
        loadDoubleLinkedListFromDatabase();
    ...
    protected void onDraw(Canvas _canvas) {
        ...
        if (mCurrentOperation == Operation.DELETE_AT && !mAnimatorDeleteAt.isRunning()) {
            mDatadoraViewModel.deleteByIdDLLAt(mLinkedListNumbers.get(mPosition)); //remove from database
            ...
            mDatadoraViewModel.deleteByIdDLLFirst(mLinkedListNumbers.get(0)); //remove from database
            ...
            mDatadoraViewModel.deleteByIdDLLLast(mLinkedListNumbers.get(
